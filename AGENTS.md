...
- `rusqlite::Connection` 会对 prepared statement 持有借用——必须在释放 `MutexGuard` 之前 drop 掉所有 statement。需要时使用作用域块（`{ let conn = ...; ... }`）将 statement 生命周期包裹起来。
...
- `capabilities/default.json` 必须包含 `global-shortcut:allow-register` 等权限，否则快捷键注册会失败。
...
- `reqwest` 使用 `rustls-tls`（非 native-tls），`rusqlite` 使用 `bundled` 特性编译 SQLite——两者都避免了对系统 OpenSSL/SQLite 的依赖，便于跨平台构建。
- 目前没有测试基础设施。

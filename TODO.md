...
- [ ] windows环境下安装后的初次启动，窗口会变得小到不可见（缩成一个透明的小方块），要人工调整才能恢复原样
...
- [x] spotlight安装后初次唤起会有多余的下方空隙，每次重启应用的第一次唤起，输入框下方的间隙要比上方大

## 2026-05-10

- [x] 保存修改好的快捷键之后显示的内容和重新打开后看的不一样
- [x] 从quickAdd和主页面add新链接之后，页面滚轮位置有差异（quickAdd之后滚轮会跳，应该和主页面一样稳定）
- [x] 导出没有导出统计信息，导入的数据不会记入本周新增
- [x] 链接可达性的更新时机？编辑后似乎没有刷新
- [x] quickAdd下方多出很多空间
- [x] 第二次打开上次有内容的quickAdd会有视觉残留
- [x] spotlight唤起闪烁问题，搜索出结果之后会轻微改变布局
...
- [x] 触碰最后打开时间应该显示具体时间
- [x] 搜索框（包括主页面和spotlight），支持用tab键循环切换选项
- [x] 按照统计信息查询的习惯，持久化到配置里
...
- [x] 新增链接可达性检查配置开关，支持在设置页关闭检测，关闭后不显示失效告警图标
- [x] 设置页布尔开关项改为滑块样式
...
- [x] 新增浏览器插件，直接从浏览器按钮一件添加链接

### 2026-05-01

- [x] 暗色模式下，搜索高亮的亮黄色和部分白色字体混在一起看不清，暗色模式下要调整高亮的颜色

### 2026-04-29

- [x] 支持导入 JSON / CSV / markdown 格式的链接

- [x] [Pass]支持从分组或标签创建链接，自动将对应的分组和链接带到新的链接上
- [x] [Pass]花里胡哨的总结性视图
- [x] [Pass]自动备份（定期导出数据库备份）

- [x] [Pass]鼠标挪到分组和标签上时，预览数量
- [x] [Pass]搜索历史记录
- [x] [Pass]链接卡片显示创建时间
- [x] [Pass]列表视图 / 卡片视图切换
- [x] [Pass]空状态引导优化（分组为空、搜索无结果时的提示）
- [x] [Pass]加载骨架屏（替代 spinner）
- [x] [Pass]侧边栏分组和标签数量统计

- [x] 扔给Links在冷启动的情况下还是会唤起主程序；
- [x] 主界面支持用上下键选中链接，回车打开，空格编辑；要注意搜索后也要继续能用上下键选中链接；
- [x] 主界面里编辑链接如果刷新，会将用户已经填好的信息覆盖掉
- [x] 每次唤起quickAdd时，应该清空输入框里的数据
- [x] 分组打开的提示，改为绿色，去掉展开搜索图标，并使得分组展开的样式，随分组层级缩进

### 2026-04-28

- [x] 新增浏览器插件，直接从浏览器按钮一件添加链接 - 临时方案：书签方式曲线救国
- [x] 表单输入体验优化（Tab 键顺序）

- [x] 支持树形结构的分组
- [x] 支持导出成浏览器支持导入的html文件（分组对应到）
- [x] "无标签的链接" 改为 "无标签"

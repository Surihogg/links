...
| `commands.rs` | 18 个 `#[tauri::command]` 函数 | 最大文件（345 行），CRUD + 搜索 + 导出 + 设置 + 导入 + 剪贴板 |
| `config.rs` | JSON 配置持久化（`HashMap<String, serde_json::Value>`） | 53 行，`get/set/save`，与 DB 同目录的 `config.json` |
| `normalize.rs` | URL 标准化（去重比较用） | 127 行，`http→https`、`www.` 去除、fragment 去除、小写 host；含单元测试 |
...
- **不要**在 `normalize_url` 中丢弃 query 参数——标准化仅用于去重比较，实际存储保留原始 URL。
